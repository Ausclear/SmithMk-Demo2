...
/// HA service — routes through local PWA on Ubuntu VM (no CORS issues)
class HAService {
  // Local PWA running on your Ubuntu VM — same network, server-side calls to HA
  static const _localPwa = 'http://192.168.1.47:3000';
  // Direct HA for image URLs only
  static const haUrl = 'http://192.168.1.101:8123';

  static Future<List<Map<String, dynamic>>> getStates() async {
    final resp = await http.get(Uri.parse('$_localPwa/api/ha/states')).timeout(const Duration(seconds: 10));
    if (resp.statusCode == 200) return List<Map<String, dynamic>>.from(json.decode(resp.body));
    throw Exception('HA states failed: ${resp.statusCode} ${resp.body}');
  }
  ...
  static Future<void> callService(String domain, String service, Map<String, dynamic> data) async {
    final resp = await http.post(Uri.parse('$_localPwa/api/ha/service'),
      headers: {'Content-Type': 'application/json'},
      body: json.encode({'domain': domain, 'service': service, 'data': data}))
      .timeout(const Duration(seconds: 10));
    if (resp.statusCode != 200) throw Exception('HA $domain/$service failed: ${resp.statusCode} ${resp.body}');
  }
  ...
  static Future<void> playSpotify(String uri, String source) async {
    final resp = await http.post(Uri.parse('$_localPwa/api/ha/play-spotify'),
      headers: {'Content-Type': 'application/json'},
