...
class MediaPage extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Column(children: [
        ...
        Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: Row(children: [
          _pill('HA', true), const SizedBox(width: 6), _pill('SUPABASE', true), const SizedBox(width: 6), _pill('SPOTIFY', true),
        ])),
        Expanded(child: LayoutBuilder(builder: (ctx, c) {
          ...
          return Center(child: Padding(padding: const EdgeInsets.all(20), child: isWide
            ? Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                _MediaTile(label: 'MUSIC', icon: PhosphorIcons.headphones(PhosphorIconsStyle.light), sub: 'Spotify · Echo · Sonos',
                  onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MusicPage()))),
                const SizedBox(width: 16), _orDivider(true), const SizedBox(width: 16),
                _MediaTile(label: 'TV', icon: PhosphorIcons.monitor(PhosphorIconsStyle.light), sub: 'Roku · NVIDIA Shield',
                  onTap: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('TV page coming soon'), backgroundColor: Color(0xFF1E1E26)))),
              ])
            : Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                _MediaTile(label: 'MUSIC', icon: PhosphorIcons.headphones(PhosphorIconsStyle.light), sub: 'Spotify · Echo · Sonos',
                  onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MusicPage()))),
                const SizedBox(height: 16), _orDivider(false), const SizedBox(height: 16),
                _MediaTile(label: 'TV', icon: PhosphorIcons.monitor(PhosphorIconsStyle.light), sub: 'Roku · NVIDIA Shield',
                  onTap: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('TV page coming soon'), backgroundColor: Color(0xFF1E1E26)))),
              ])));
        })),
  ...
  static Widget _orDivider(bool vertical) {
    if (vertical) {
      return Column(mainAxisSize: MainAxisSize.min, children: [
        Container(width: 1, height: 30, decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter,
          colors: [Colors.transparent, Colors.white.withValues(alpha: 0.08), Colors.transparent]))),
        const Padding(padding: EdgeInsets.symmetric(vertical: 6), child: Text('OR', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 2, color: Color(0x26FFFFFF)))),
        Container(width: 1, height: 30, decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter,
          colors: [Colors.transparent, Colors.white.withValues(alpha: 0.08), Colors.transparent]))),
      ]);
    }
    return Row(mainAxisSize: MainAxisSize.min, children: [
      Container(height: 1, width: 30, decoration: BoxDecoration(gradient: LinearGradient(
        colors: [Colors.transparent, Colors.white.withValues(alpha: 0.08), Colors.transparent]))),
      const Padding(padding: EdgeInsets.symmetric(horizontal: 8), child: Text('OR', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 2, color: Color(0x26FFFFFF)))),
      Container(height: 1, width: 30, decoration: BoxDecoration(gradient: LinearGradient(
        colors: [Colors.transparent, Colors.white.withValues(alpha: 0.08), Colors.transparent]))),
    ]);
  }
...
/// Pressable 3D media tile with scale + glow animation on tap
class _MediaTile extends StatefulWidget {
  final String label, sub;
  final IconData icon;
  final VoidCallback onTap;
  const _MediaTile({required this.label, required this.icon, required this.sub, required this.onTap});
  @override
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: _pressed ? SmithMkColors.accent.withValues(alpha: 0.4) : Colors.white.withValues(alpha: 0.07)),
          boxShadow: _pressed ? [
            // Pressed — inset shadow, no lift
            BoxShadow(color: Colors.black.withValues(alpha: 0.6), blurRadius: 12, offset: const Offset(2, 2)),
            BoxShadow(color: SmithMkColors.accent.withValues(alpha: 0.15), blurRadius: 30),
          ] : [
        ...
        child: Stack(children: [
          ...
          // Glow on press
          if (_pressed) Positioned.fill(child: Container(
            decoration: BoxDecoration(borderRadius: BorderRadius.circular(24),
              gradient: RadialGradient(center: Alignment.topCenter, radius: 1.2,
                colors: [SmithMkColors.accent.withValues(alpha: 0.12), Colors.transparent])))),
          // Content
            ...
            AnimatedContainer(
              ...
              decoration: BoxDecoration(shape: BoxShape.circle,
                gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight,
                  colors: _pressed ? [const Color(0xFF2A2210), const Color(0xFF1E1A0E)] : [const Color(0xFF222222), const Color(0xFF161616)]),
                boxShadow: [
                  if (!_pressed) BoxShadow(color: Colors.black.withValues(alpha: 0.6), blurRadius: 16, offset: const Offset(6, 6)),
                  if (!_pressed) BoxShadow(color: Colors.white.withValues(alpha: 0.02), blurRadius: 8, offset: const Offset(-3, -3)),
                  if (_pressed) BoxShadow(color: SmithMkColors.accent.withValues(alpha: 0.2), blurRadius: 20),
                ]),
